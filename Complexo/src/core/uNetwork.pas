...
begin
  ...
  begin
    raise EExecaoElementos.Create(('Não é possivel adcionar mais elementos no conjunto'));
  end;

  if vgContador < vgQtdElementos then
  begin
    vgElementos[vgContador] := APrimeiroElementosCnct;
    ShowMessage('Número: ' + IntToStr(vgElementos[vgContador]) + ', adcionado no conjunto.');
  ...
  begin
    ...
    if vgContadorQtdElementos > 1 then
      ShowMessage('Digite mais números para completar a quantidade de elementos do conjunto.');
...
begin
  try
    ...
    if APrimeiroElementosCnct <> ASegundoElementosCnct then
      raise EExecaoElementos.Create(('Elementos não Conectados'));
